...
```\
\
### Explicit constructor options [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#explicit-constructor-options)\
\
...
18\
19\
const supabase = createClient(apiURL, apiKey, {    db: {      schema: 'public',    },    auth: {      storage: AsyncStorage,      autoRefreshToken: true,      persistSession: true,      detectSessionInUrl: true,    },    realtime: {      channels,      endpoint,    },    global: {      fetch: customFetch,      headers: DEFAULT_HEADERS,    },  })\
```\
\
### TypeScript support [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#typescript-support)\
\
...
1\
2\
// previously definitions were injected in the `from()` method  supabase.from<Definitions['Message']>('messages').select('\*')\
```\
\
* * *\
\
...
5\
6\
import type { Database } from './DatabaseDefinitions'  // definitions are injected in `createClient()`  const supabase = createClient<Database>(SUPABASE_URL, ANON_KEY)  const { data } = await supabase.from('messages').select().match({ id: 1 })\
```\
\
...
1\
2\
supabase startsupabase gen types typescript --local > DatabaseDefinitions.ts\
```\
\
### Data operations return minimal [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#data-operations-return-minimal)\
\
...
```\
\
### New ordering defaults [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#new-ordering-defaults)\
\
...
\
### Cookies and localstorage namespace [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#cookies-and-localstorage-namespace)\
\
...
```\
\
### New Auth Types [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#new-auth-types)\
\
...
8\
9\
interface Session {  provider_token?: string | null  access_token: string  expires_in?: number  expires_at?: number  refresh_token: string  token_type: string  user: User}\
```\
\
### New Auth methods [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#new-auth-methods)\
\
...
3\
4\
const { data } = await supabase.auth.signIn({    email: 'hello@example',    password: 'pass',  })\
```\
\
* * *\
\
...
3\
4\
const { data } = await supabase.auth.signInWithPassword({    email: 'hello@example',    password: 'pass',  })\
```\
\
### New Realtime methods [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#new-realtime-methods)\
\
...
```\
\
### Deprecated setAuth() [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#deprecated-setauth)\
\
...
\
### All changes [\#](https://supabase.com/docs/reference/javascript/auth-signinwithoauth\#all-changes)\
\
- `supabase-js`\
