...
if [ "$1" = "makemigrations" ]; then
    ...
    docker exec -it $CONTAINER_NAME python /app/manage_db.py makemigrations "$2"
elif [ "$1" = "migrate" ]; then
    docker exec -it $CONTAINER_NAME python /app/manage_db.py migrate
elif [ "$1" = "showmigrations" ]; then
    docker exec -it $CONTAINER_NAME python /app/manage_db.py showmigrations
else
